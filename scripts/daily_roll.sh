...
#      in production remove this line — the backend inserts demand/flu/inventory rows)
#   2. full-chain rollover from PostgreSQL: forecasts → replenishment → transfers → simulation
#      → alerts, everything written to PostgreSQL [OUTPUT] tables
set -euo pipefail
cd "$(dirname "$0")/.."
PY=.venv/bin/python
